...
typedef struct clap_clap_converter {
   ...
   // [main-thread]
   CLAP_NODISCARD bool (*convert_state)(const struct clap_clap_converter *converter,
                                        const clap_istream_t             *src,
                                        const clap_ostream_t             *dst);

   // converts the clap param id and normalized value.
   // [thread-safe]
   CLAP_NODISCARD bool (*convert_normalized_value)(const struct clap_clap_converter *converter,
   ...
   // [thread-safe]
   CLAP_NODISCARD bool (*convert_plain_value)(const struct clap_clap_converter *converter,
                                              clap_id                           src_param_id,
...
typedef struct clap_clap_converter_factory {
   ...
   CLAP_NODISCARD uint32_t (*count)(const struct clap_clap_converter_factory *factory);

   // Get the converter at the given index
   CLAP_NODISCARD const
      clap_clap_converter_t *(*get)(const struct clap_clap_converter_factory *factory,
                                    uint32_t                                  index);
