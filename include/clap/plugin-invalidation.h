...
typedef struct clap_plugin_invalidation_factory {
   ...
   CLAP_NODISCARD uint32_t (*count)(const struct clap_plugin_invalidation_factory *factory);

   // Get the invalidation source by its index.
   // [thread-safe]
   CLAP_NODISCARD const clap_plugin_invalidation_source_t *(*get)(
   ...
   CLAP_NODISCARD bool (*refresh)(const struct clap_plugin_invalidation_factory *factory);
